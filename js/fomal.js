...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | Achang☀️")[0];
}
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by Achang☀️ %c 你正在访问Achang☀️の小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| Achang☀️") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('Achang☀️的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
function createtime() {
  ...
  var start = new Date("04/14/2025 21:30:00"); // 旅行者1号开始计算的时间
  var dis = Math.trunc(25000000000 + ((now - start) / 1000) * 17); // 距离=秒数*速度 记住转换毫秒
  var unit = (dis / 149600000).toFixed(6);  // 天文单位
  // 网站诞生时间
  var grt = new Date("04/14/2025 21:30:00");
  ...
  (currentTimeHtml =
    hnum < 20 && hnum >= 11
      ? `<img class='boardsign' src='https://achang0721.github.io/assets/昌想屋-科研摸鱼中.svg' title='什么时候能够实现财富自由呀~' style="width:12%"><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`
      : `<img class='boardsign' src='https://achang0721.github.io/assets/昌想屋-下班休息啦.svg' title='下班了就该开开心心地玩耍~' style="width:12%"><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`),
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://source.fomal.cc/img/default_cover_5.webp);
    --darkmode-bg:url(https://source.fomal.cc/img/default_cover_8.webp);
